...
The first complete dry run found the fourth, and it is the same shape one level
down: `wix build` harvested zero files, said so as a warning, and produced an
MSI that installed an empty directory. Issue #86. A step that finds nothing now
