...
export function NavRail({ sections, current, onSelect, collapsed, icon, label = 'Primary', testId = 'left-nav', }) {
    ...
    return (_jsx("nav", { className: `nav${collapsed ? ' nav--collapsed' : ''}`, "aria-label": label, "data-testid": testId, children: sections.map((section, index) => (_jsxs(Collapsible.Root, { className: "nav__section", open: collapsed ? true : (open[section.id] ?? true), onOpenChange: (next) => setOpen((prev) => ({ ...prev, [section.id]: next })), children: [collapsed ? (
                // The heading's space, kept. Removing it shifted every icon below.
                _jsx("span", { className: "nav__break", "data-first": index === 0 || undefined })) : (_jsxs(Collapsible.Trigger, { className: "nav__heading", children: [_jsx(ChevronDown, { className: "nav__chevron", size: 12 }), _jsx("span", { children: section.label })] })), _jsx(Collapsible.Content, { children: section.items.map((entry) => {
